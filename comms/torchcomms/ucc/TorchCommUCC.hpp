...
class TorchCommUCC : public TorchCommBackend,
                     public std::enable_shared_from_this<TorchCommUCC> {
 public:
  static constexpr std::string_view kBackendName = "ucc";

  TorchCommUCC();
  ~TorchCommUCC() override;

  TorchCommUCC(const TorchCommUCC&) = delete;
  TorchCommUCC(TorchCommUCC&&) = delete;
  TorchCommUCC& operator=(const TorchCommUCC&) = delete;
  TorchCommUCC& operator=(TorchCommUCC&&) = delete;
 ...
 private:
  ...
  // UCC helpers
  static ucc_datatype_t toUCCDatatype(at::ScalarType type);
  static ucc_reduction_op_t toUCCReduceOp(const ReduceOp& op);
  void uccCheck(ucc_status_t status, const char* msg);

  // OOB allgather for UCC team creation (uses store)
  static ucc_status_t oobAllgather(
      void* sbuf,
      void* rbuf,
      size_t msglen,
      void* coll_info,
      void** req);
  static ucc_status_t oobAllgatherTest(void* req);
  static ucc_status_t oobAllgatherFree(void* req);
  ...
  // OOB info for UCC context/team — must outlive ctx_ and team_ since UCC
  // may call OOB callbacks during cleanup.  Declared before UCC handles so
  // it is destroyed after them (reverse declaration order).
  struct OOBCollInfo;
  std::unique_ptr<OOBCollInfo> oob_info_;

  // UCC handles
  ucc_lib_h lib_{nullptr};
  ucc_context_h ctx_{nullptr};
  ucc_team_h team_{nullptr};

  uint32_t p2pCounter_{0};
